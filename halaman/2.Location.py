...
for parameter in filtered_data['Parameter'].unique():
    ...
    plt.plot(filtered_data[filtered_data['Parameter'] == parameter]['Tahun'].dt.month, filtered_data[filtered_data['Parameter'] == parameter]['Konsentrasi'])
    plt.xlabel('Bulan')
    plt.ylabel('Konsentrasi')
    plt.title(f'Grafik {parameter} di Lokasi {selected_location} pada Tahun {selected_year}')
